...
type Response = Either<
  AppErrors.EntityNotFoundError | AppErrors.InvalidPayloadError | TripErrors.TripDoesNotBelongToUser | TripErrors.TripDayPositionSequenceInvalidError,
  Result<void>
>;
  ...
  execute = async (payload: UpdateTripDayPositionDTOWithUserId): Promise<Response> => {
    try {
      ...
      if(!isValidPositionArray(trip.days.length, request.positions)) {
        return left(new TripErrors.TripDayPositionSequenceInvalidError());
      }
