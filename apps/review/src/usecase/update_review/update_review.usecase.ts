...
type UpdateReviewDTOWithUserId = {
    userId: string;
    request: UpdateReviewDTO;
}

@Injectable()
export class UpdateReviewUseCase implements UseCase<UpdateReviewDTOWithUserId, Promise<Response>>
{
  constructor(
  ...
  execute = async (payload: UpdateReviewDTOWithUserId): Promise<Response> => {
    try {
      ...
      reviewOrError.title = request.title;
      reviewOrError.description = request.description;
      reviewOrError.rating = request.rating;
      reviewOrError.updatedAt = new Date();
