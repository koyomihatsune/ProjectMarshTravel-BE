...
import { UpdateCommentDTO } from './update_comment.dto';
...
type UpdateCommentDTOWithUserId = {
    userId: string;
    request: UpdateCommentDTO;
}

@Injectable()
export class UpdateCommentUseCase implements UseCase<UpdateCommentDTOWithUserId, Promise<Response>>
{
  constructor(
  ...
  execute = async (payload: UpdateCommentDTOWithUserId): Promise<Response> => {
    try {
      ...
      commentOrError.content = request.content;
